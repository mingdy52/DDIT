...
<script src="http://cdn.ckeditor.com/4.19.1/basic/ckeditor.js"></script>
<style>
...
<form:form modelAttribute="myreview" method="post" action="${cPath }/myexpert/myreview/${myreview.reviewNum }" >
<table class="table table-borderless mb-0">
	<tr>
		<td><form:input path="reviewNum" value="${myreview.reviewNum}" type="hidden"/></td>
	</tr>
	<tr>
		<td><form:input path="myEprod" class="form-control" value="${myreview.myEprod}"  type="hidden"/></td>
	</tr>
	<tr>
		<td><form:input path="reviewTitle" id="title" value="${myreview.reviewTitle }" placeholder="제목을 입력하세요.."/></td>
	</tr>
	<tr>
		<td><form:input path="reviewContent" class="form-control" style="width:100%; min-height:200px;"  value="${myreview.reviewContent }"/></td>
	</tr>
	<tr>
...
<script>
	let rate = $("input[name='reviewStar']").val();
	console.log(rate);

	$("input[name=rating][value="+ rate +"]").trigger("click");
