 ...
 *  - hashCode()는 객체에 대한 해시코드값을 반환하는 메서드다.
 *  	=> 해시테이블 생성시 사용됨.
 *  
 *  - equals() 와 hashCode()에 관련된 규칙(Convention)
 *    1. 두 객체가 같으면 반드시 같은 hashCode를 가져야 한다.
 *    2. 두 객체가 같으면 equal()를 호출했을 때 true를 반환해야 한다.
 *  	  즉, 객체 a와 b가 같다면 a.equals(b)와 b.equals(a) 둘다 true 여야 한다.
 ...
 *    	  한 정수 값을 반환한다. 그러므로, 클래스에서 hashCode()를 override하지 않으면
