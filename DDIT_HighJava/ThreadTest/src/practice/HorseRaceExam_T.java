...
public class HorseRaceExam_T {

	public static int rank = 1; // 현재 말의 순위

	public static List<Horse3> horseList;

	public static void main(String[] args) {
		
		// 고정된 크기의 리스트 객체 생성하기
		horseList = Arrays.asList(new Horse3("01번말"), new Horse3("02번말"), new Horse3("03번말"), new Horse3("04번말"),
...
class Horse3 extends Thread implements Comparable<Horse3> {
	...
	public void run() {
		...
		this.rank = HorseRaceExam_T.rank;
		HorseRaceExam_T.rank++;
	}
}
...
class HorsePositionDisplay extends Thread {
	...
	public void run() {
		...
		while (true) {
			...
			for (int i = 0; i < HorseRaceExam_T.horseList.size(); i++) {

				String horseCourse = "--------------------------------------------------"; // 50구간
				Horse3 horse = HorseRaceExam_T.horseList.get(i);
